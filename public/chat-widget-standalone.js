...
// Chat Widget Standalone Implementation
(function() {
  console.log('Chat widget initialized with config:', window.PULLSE_CHAT_CONFIG);
  
  // Import the Chat Widget API
  const ChatWidgetAPI = window.pullseChatAPI || {
  ...
  ChatWidgetAPI.initializeChatWidget({
    workspaceId: workspaceId,
    theme: config.theme || {},
    position: config.theme?.position || 'right',
    compact: config.theme?.compact || false,
    ...
    events: {
      ...
      onWidgetOpened: () => {
        ...
        document.getElementById('pullse-chat-loading').style.display = 'none';
      },
      onWidgetClosed: () => {
  ...
  function setupChatUI() {
    ...
    const renderConversationView = async () => {
      // Get messages from API if the conversation exists
      if (currentConversation === 'recent') {
        try {
          const fetchedMessages = await ChatWidgetAPI.getMessages();
          if (fetchedMessages && fetchedMessages.length > 0) {
            messages = fetchedMessages;
          } else if (messages.length === 0) {
            // Fallback demo messages if none were fetched
            messages = [
              { id: 1, sender: 'user', content: 'can you explain copilot to me', timestamp: '1d ago' },
              { id: 2, sender: 'agent', content: 'GitHub Copilot is an AI-powered code completion tool that helps developers write code faster and with fewer errors. It suggests code snippets based on comments and existing code, supporting many programming languages. Would you like to know more about specific features?', timestamp: '1d ago' }
            ];
          }
        } catch (error) {
      ...
      const messagesHtml = messages.map(msg => {
        ...
        const messageText = msg.content || msg.text; // Support both API format and local format
        const messageTime = msg.timestamp || msg.time || 'Just now';
        ...
        return `
          <div class="flex ${isUser ? 'justify-end' : 'justify-start'} mb-4">
            <div class="${isUser ? 'bg-primary text-white' : 'bg-gray-700 text-white'} px-3 py-2 rounded-lg ${isUser ? 'rounded-br-none' : 'rounded-bl-none'} max-w-[80%]">
              <div>${messageText}</div>
              <div class="text-xs mt-1 ${isUser ? 'text-white/70' : 'text-gray-400'}">${messageTime}</div>
            </div>
          </div>
        `;
      }).join('');
    ...
    const sendMessage = async () => {
      ...
      try {
        ...
        await ChatWidgetAPI.sendMessage(text);
        
        // In a real implementation, we would listen for the response from server
        // For demo purposes, simulate an agent response after a delay
        setTimeout(async () => {
          messages.push({
            id: Date.now() + 1,
            sender: 'agent',
            content: `Thanks for your message! I'll help you with "${text}". Is there anything specific you'd like to know?`,
            timestamp: 'Just now'
          });
  ...
  window.pullseChatAPI = ChatWidgetAPI;
