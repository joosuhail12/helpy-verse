...
export const VersionHistory = ({ versions, onRestore, currentVersionId }: VersionHistoryProps) => {
  ...
  const getUserInfo = (version: ContentVersion) => {
    return version.user || version.createdBy || { name: 'Unknown', avatar: '' };
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          {versions.map((version) => {
            ...
            const user = getUserInfo(version);
          ...
          {selectedVersion && (
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                <Avatar className="h-8 w-8">
                  <AvatarImage 
                    src={getUserInfo(selectedVersion)?.avatar} 
                    alt={getUserInfo(selectedVersion)?.name} 
                  />
                  <AvatarFallback>
                    {getUserInfo(selectedVersion)?.name?.charAt(0)}
                  </AvatarFallback>
                </Avatar>
                <div>
                  <div className="font-medium">
                    {getUserInfo(selectedVersion)?.name}
                  </div>
                ...
                <div className="p-3 bg-muted rounded-md">
                  ...
                  <div className="text-sm">
                    {selectedVersion.changeDescription || (
                      typeof selectedVersion.changes === 'string' ? (
                        selectedVersion.changes
                      ) : (
                        <ul className="list-disc pl-5">
                          {Array.isArray(selectedVersion.changes) && selectedVersion.changes.map((change, idx) => (
                            <li key={idx}>
